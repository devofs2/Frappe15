...
def send_via_gateway(arg):
    ...
    # Form the request payload as per Infobip requirements
    args = {
        "messages": [
            {
                "from": ss.sender_name if hasattr(ss, 'sender_name') and ss.sender_name else "OWO Sante",
                "destinations": [{"to": d} for d in arg.get("receiver_list")],
                "text": message
            }
        ]
    }
    ...
    if 200 <= status < 300:
        success_list.extend(arg.get("receiver_list"))
