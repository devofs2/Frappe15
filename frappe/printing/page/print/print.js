...
frappe.ui.form.PrintView = class {
	...
	show(frm) {
		...
		// Rendre les logos dynamique lors des impressions 
		/* this.update_letter_header().then(() => {
			return frappe.run_serially(tasks);
		}).catch(err => {
			console.error("Erreur lors de la mise à jour du Letter Head :", err);
		}); */
		
	}

	update_letter_header() {
		return new Promise((resolve, reject) => {
			let current_user = frappe.session.user;
			let user_company = frappe.defaults.get_user_default("company");
	
			if (user_company) {
				// Récupération de la fiche de la société pour obtenir le logo
				frappe.call({
					...
					callback: function(r) {
						if (r.message) {
							...
							if (company_logo) {
								// Recherche du Letter Head par défaut (is_default = 1)
								frappe.call({
									...
									callback: function(r2) {
										if (r2.message && r2.message.length) {
											let letterhead = r2.message[0];
											// Récupération de tous les fichiers attachés à ce Letter Head
											frappe.call({
												method: "frappe.client.get_list",
												args: {
													doctype: "File",
													filters: {
														attached_to_doctype: "Letter Head",
														attached_to_name: letterhead.name
													},
													fields: ["name"]
												},
												callback: function(r3) {
													let deletions = [];
													// Pour chaque fichier attaché, on le supprime
													(r3.message || []).forEach(file => {
														deletions.push(new Promise((resolve_del, reject_del) => {
															frappe.call({
													...
													// Une fois tous les fichiers supprimés, on met à jour le Letter Head
														...
														.then(() => {
															frappe.call({
																...
																callback: function(r4) {
																	//console.log(__("Lettre en-tête mise à jour avec succès."));
																	resolve();
																},
																error: function(err) {
																	console.error("Erreur lors de la mise à jour du Letter Head :", err);
												...
												error: function(err) {
													console.error("Erreur lors de la récupération des fichiers attachés :", err);
													reject(err);
												}
											});
										} else {
											console.log(__("Aucun Letter Head par défaut trouvé."));
											resolve();
										}
									},
									error: function(err) {
										console.error("Erreur lors de la recherche du Letter Head :", err);
										reject(err);
									}
								});
							} else {
								console.log(__("Aucun logo n'est défini pour la société " + user_company));
								resolve();
							}
						} else {
							console.log(__("Impossible de récupérer la fiche de la société " + user_company));
							resolve();
						}
					},
					error: function(err) {
				...
				console.log(__("Aucune société n'est assignée à l'utilisateur " + current_user));
				resolve();
			}
		});
	}
