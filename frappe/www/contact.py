...
def get_context(context):
	assureurs =  frappe.db.sql(
        """SELECT * FROM `tabCompany`
        WHERE type_company  = %s
        ORDER BY name ASC""",
        ("Insurer",),
        as_dict=True,
	...
	query_options = ["Entretien", "Assistance/Support", "Ventes"]

	out = {"query_options": query_options, "assureurs":assureurs, "parents": [{"name": _("Home"), "route": "/"}]}
